...
#info
    if message.content == f"{前輟}info":
        await message.delete()
        embed = discord.Embed(title="關於", description=f"prefix是{前輟}", color=0x04f108)
        embed.add_field(name="每日簽到最高", value=f"{MAX}$")
        embed.add_field(name="每日簽到最低", value=f"{MIN}$")
...
#創建帳號
    if message.content == f"{前輟}new":
      ...
      if os.path.isfile(filepath):
          await message.channel.send(f"{message.author.mention}您已建立帳號了!")
      else:
...
#簽到
    if message.content == f"{前輟}sign":
        ...
        if os.path.isfile(filepath):
            ...
            await message.channel.send(f"{message.author.mention}您沒有帳號,請建立帳號 , [`{前輟}!new`創建一個]")
#查詢
